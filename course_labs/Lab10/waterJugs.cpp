...
void build_graph(void) {
   ...
   state ourState = make_pair(0, 0);
   int A = 3; 
   int B = 4;
   state last = make_pair(A, B);
      ...
      while (ourState != last)   {
          ...
          incState = make_pair(A, ourState.second);              // Fill A

            if(incState != ourState)   {

                pair<state,state> edge = make_pair(ourState, incState);
                nbrs[ourState].push_back(incState);
                edge_label[edge] = actions[0];

            }


          incState = make_pair(ourState.first, B);
          ...
          incState = make_pair(0, ourState.second);
          ...
          incState = make_pair(ourState.first, 0);
          ...
          int updateA = ourState.first;
          int updateB = ourState.second;


          if(ourState.second < B)   {

              while(updateA > 0 && updateB < B)   {
              ...
              incState = make_pair(updateA,updateB);
          ...
          updateA = ourState.first;
          updateB = ourState.second;


          if(ourState.first < A)   {

              while((updateA < A) && (updateB > 0))   {
          ...
          if(ourState.second == B)
              ourState = make_pair(ourState.first + 1,0);
          else
              ourState = make_pair(ourState.first, ourState.second + 1);
